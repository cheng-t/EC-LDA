...
def get_gradient_dummy_random(args,model,feature_size,dummy_node_size=100,dummy_link_p=0.004):
    ...
    transform = T.RandomLinkSplit(num_val=0,num_test=0,is_undirected=True,add_negative_train_samples=False,neg_sampling_ratio=0.0)
    train,val,test = transform(dummy_g)
    dummy_g = train
    neg_edge_index = negative_sampling(
        edge_index=dummy_g.edge_index, num_nodes=dummy_g.num_nodes,
        num_neg_samples=int(dummy_g.edge_label_index.size(1)*1), method='sparse')
    ...
    edge_label = torch.cat([
        dummy_g.edge_label,
        dummy_g.edge_label.new_zeros(neg_edge_index.size(1))
    ], dim=0)
    ...
    for i in range(local_iterations):
        ...
        orig_out = model(dummy_x,dummy_g.edge_index,edge_label_index)
        loss = loss_f(orig_out,edge_label)
