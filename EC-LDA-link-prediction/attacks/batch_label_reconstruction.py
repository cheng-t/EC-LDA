...
from torch_geometric.utils import erdos_renyi_graph,negative_sampling
...
def get_p_and_O(model,dummy_g):
    ...
    transform = RandomLinkSplit(num_val=0,num_test=0,is_undirected=True,add_negative_train_samples=False)
    dummy_g ,_,__ = transform(dummy_g)
    
    neg_edge_index = negative_sampling(
        edge_index=dummy_g.edge_index, num_nodes=dummy_g.num_nodes,
        num_neg_samples=dummy_g.edge_label_index.size(1), method='sparse')

    edge_label_index = torch.cat(
        [dummy_g.edge_label_index, neg_edge_index],
        dim=-1,
    )

    # 此时已经得到O的值
    output = model(dummy_g.x,dummy_g.edge_index,edge_label_index)
...
def set_y(dummy_node_size,dis):
    ...
    return res_tensor
                                                                                                            ...
                                                                                                            #  0.004
def batch_label_construction(train_data,args,model,gradients,num_links,feature_size,dis_y,dummy_node_size=1000,dummy_link_p=0.004):
    if train_data == None:
        dummy_x = torch.normal(mean=0.0,std=0.001,size=(dummy_node_size,feature_size)).to(args.device).requires_grad_(True)
        # dummy_y = torch.normal(mean=0.0,std=0.001,size = (dummy_node_size,args.num_classes)).to(args.device).requires_grad_(True)
        ...
        dummy_g = Data(x=dummy_x,edge_index=edge_index)
    else:
        dummy_g = train_data
    
    ps,Os = get_p_and_O(copy.deepcopy(model),dummy_g)
    p = ps.clone().detach()
    ps = ps.mean(dim=0)
    ...
    # O = 5.5
    # tmp1 = Os.sum(dim=1)
    # tmp2 = torch.Tensor([O for i in range(Os.shape[0])])
    # tmp3 = (torch.Tensor([O for i in range(Os.shape[0])])-Os.sum(dim=1))*1e6
    ...
    # ps = torch.tensor([1/args.num_classes for i in range(args.num_classes)])
    ...
    abs_ = sum(abs(torch.Tensor([O.cpu() for i in range(Os.shape[0])])-Os.cpu().sum(dim=1)))
    return counts.cpu().numpy(),O.cpu(),(abs_).cpu(),(tmp4*1e5).cpu()
