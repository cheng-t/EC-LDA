...
from dataset.generate_LastFMAsia import generate_LatsFMAsia
import os
...
class Server():
    ...
    def get_dataset(self):
        ...
        if self.dataset == 'Cora':
            ...
            client_data,self.args.num_classes = generate_LatsFMAsia(self.num_clients,self.split,self.rand_seed)
        else:
    ...
    def receive_models(self):  # client->server
        ...
        for client in self.selected_clients:
            tot_train_links += client.train_links_num
            self.uploaded_ids.append(client.id)
            self.uploaded_weights.append(client.train_links_num)
            self.uploaded_models.append(client.model)

        for i, w in enumerate(self.uploaded_weights):  # 权重归一化
            # 每个上传client的权重
            self.uploaded_weights[i] = w / tot_train_links    
    ...
    def train_metrics(self):
        ...
        links = []
        for c in self.clients:
            cl, ls ,correct_links= c.train_metrics()
            num_samples.append(ls)
            links.append(correct_links)
            losses.append(cl * 1.0)

        ids = [c.id for c in self.clients]
        return ids, num_samples, losses, links
