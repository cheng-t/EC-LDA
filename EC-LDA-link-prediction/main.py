...
def run(args,i):
    ...
    if args.dataset in ['Cora','CiteSeer','PubMed','FacebookPagePage','WikiCS','CoraFull','Reddit2','LastFM']:
        args.model = algorithm(args.dataset,args.gcn_hops,features_in=args.feature_size).to(args.device)
    else:
...
if __name__ == '__main__':
    ...
    # 是否进行主动攻击
    parser.add_argument('-aa','--active_attack',type=bool,default=not False,
    ...
    # 数据集
    parser.add_argument('-data', "--dataset", type=str, default="Cora")     # Cora,CiteSeer,PubMed,FacebookPagePage,CoraFull
    ...
    # 生成的negative边是positive边的多少倍
    parser.add_argument('-nt','--neg_times',type=float,default= 1,
                        help = 'all train_link_num = pos_link + pos_link * neg_times')

    # learning rate,GCN:0.01   GraphSAGE:0.05
    ...
    parser.add_argument('-agr','--attack_global_round',type=list,default=[i for i in range(0,100,2)])
    ...
    # # 是否进行对比
    # parser.add_argument('-comp','--compare',type=bool,default=True,
    #                     help = "compare with other method or not")
    
    # # 要对比算法的名字
    # parser.add_argument('-cn','--compare_name',type=str,default='iLRG',
    #                     help='select which methods to compare with')            # iLRG, LLG_star, ALDIA_no_active
    ...
    # 攻击时期
    parser.add_argument('-ap','--attack_period',type = str,choices=['start','mid','end'])
    ...
    # task ALDA  compare  defense
    parser.add_argument('-ta','--task',type=str,default = 'ALDA')

    # 采样率
    parser.add_argument('-sr','--sample_rate',type=float,default=1)

    # 隐私预算
    parser.add_argument('-eps','--epsilon',type=float,default=2)
    ...
    if args.task == 'ALDA' or args.task == 'defense' or args.task == 'compare':
    ...
    # ['PubMed',            0.01,   200,    0.01,   'Adam'], 原来的Adam配置
    # 默认设置下的
    #       数据集,       学习率,  global_rounds, clip norm, 优化器
    default_setting = [
        ['Cora',              1,      200,    0.01,   'SGD'],
        # ['PubMed',            0.01,   200,    0.01,   'Adam'],
        ['FacebookPagePage',  0.1,    300,    0.01,   'SGD'],
        ['CiteSeer',          0.5,    300,    0.01,   'SGD'],
        ['WikiCS',            0.6,    300,    0.01,   'SGD'],
        ['CoraFull',          1,      300,    0.01,   'SGD'],
        ['LastFM',            0.5,    100,    0.01,   'SGD']
    ]

    for row in default_setting:
        if args.dataset == row[0]:
            args.learning_rate = row[1]
            args.global_rounds = row[2]
    ...
    if args.algorithm == 'GraphSAGE' and args.dataset == 'CoraFull':
        args.clip_norm = 0.1

    if args.attack_period == 'start' and (args.dataset == 'PubMed' or args.dataset == 'Cora'):
        args.clip_norm = 100 
    ...
    if not args.compare:
        ...
        print('cos_sim')
        print('LDA:',avg_cos_sim_no_active)
        print('iLRG:',avg_cos_sim_iLRG)
        print('LLG_star:',avg_cos_sim_LLG_star)

        print('js_div')
        print('LDA:',avg_js_div_no_active)
        print('iLRG:',avg_js_div_iLRG)
        print('LLG_star:',avg_js_div_LLG_star)
        print("all_final_acc:",final_acc)

        print('avg_cos_sim')
        print('LDA:{: .3f}'.format(sum(avg_cos_sim_no_active)/len(avg_cos_sim_no_active)))
        ...
        print('js_div')
        print('LDA:{: .3f}'.format(sum(avg_js_div_no_active)/len(avg_js_div_no_active)))
